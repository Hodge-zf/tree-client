...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    //Used to display a message dialog and is not part of the solution        
        ...
        if(firstUse)
        {
            ...
            "Sample application for encoding text using a tree");
            firstUse = false;
        }
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< 
    }
...
//>>>>>> MODIFIED   >>>>>>>>>>>>>>>>>>>>>>>>>>>>
        return "Encode a file using Huffman Coding (Sample)";
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    }
...
//>>>>>> ADDED CODE >>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ...
        // Make sure the coder is reset
        coder.resetCurrentNode();
        
        while(!coder.isSingleSymbol())
        {
            //Check left for symbol
            if(coder.checkLeft(c))
            {
                // Found it left
                codeBuffer.addCharacter(new Character('0'));
                coder.advanceLeft();
            }
            else
            {
                codeBuffer.addCharacter(new Character('1'));
                coder.advanceRight();
            }
        ...
        // At the end of the tree, we are done 
